...
function ColumnEnd() {
  ...
  let [users] = useState(
    userData.users.sort((a, b) => b.nberPosts - a.nberPosts)
  );
  //Store topic
  let topics = [];
  //Store posts
...
function changeRowColor(tableName, tbleHeader) {
  ...
  let sibRow =
    tbl.getElementsByTagName("tr")[tbl.getElementsByTagName("tr").length - 2];
  let thead = tbl.getElementsByClassName(`${tbleHeader}`)[0];
  let row =
    tbl.getElementsByTagName("tr")[tbl.getElementsByTagName("tr").length - 1];
